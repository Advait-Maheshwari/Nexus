...
export function MetricTile({ metric, index }: { metric: Metric; index: number }) {
  return (
    <motion.article
      className="glass-panel scanline min-w-0 rounded-lg p-4"
    ...
    >
      ...
      <div className="mt-3 flex min-w-0 flex-wrap items-end justify-between gap-2">
        <strong className={cn("min-w-0 text-3xl font-semibold", toneClass[metric.tone])}>
          {metric.value}
        </strong>
        <span className="max-w-full truncate rounded-full border border-white/10 bg-white/[0.05] px-2 py-1 text-xs text-slate-300">
          {metric.delta}
        </span>
      </div>
    </motion.article>
